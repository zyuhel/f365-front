...
<template>
  <main>
    <h3><router-link :to="`/day/${dateObject.day-1}`" v-if="dateObject.day>1">&lt;</router-link><span v-else>&lt;</span> День {{dateObject.day}} ({{dateObject.date}}) <router-link :to="`/day/${parseInt(dateObject.day)+1}`" v-if="voteFinished || dateObject.day<3">&gt;</router-link><span v-else>&gt;</span> <span v-if="!voteFinished && dateObject.day > 2"> - Голосование еще не завершено</span><span v-else-if="!voteFinished"> - Голосование не проводилось</span></h3>
